...
class LicenseManager:
    ...
    def __init__(self, cache_dir: Path):
        ...
        # Gumroad product ID for license verification
        # This is the product_id from your Gumroad product settings
        # Set via environment variable GUMROAD_PRODUCT_ID if you need to change it
        self.gumroad_product_id = os.getenv(
            "GUMROAD_PRODUCT_ID",
            "XKHXLhf4IyWJvlL3rMBI0w=="  # Your actual Gumroad product ID
        )
    ...
    def activate_license(self, key: str, email: str) -> tuple[bool, str]:
        ...
        # Try Gumroad API verification first
        success, message, gumroad_data = self._verify_gumroad_license(key)
        if success:
            # Use email from Gumroad data if available, otherwise use provided email
            license_email = gumroad_data.get("purchase", {}).get("email", email) if gumroad_data else email
            ...
            return True, f"License activated successfully! Welcome to ESHU Premium."

        # Fallback to offline validation for demo/development keys
        if self._validate_key_format(key):
            if self._verify_key_offline(key, email):
                ...
                self.save_license(license)
                return True, "License activated successfully! (Demo mode)"

        return False, f"Invalid license key. {message}"

    def _verify_gumroad_license(self, license_key: str) -> tuple[bool, str, Optional[Dict]]:
        """
        Verify license key with Gumroad API
        Returns: (success, message, gumroad_data)
        """
        try:
            ...
            if response.status_code == 200 and data.get("success"):
                ...
                # Check if license is valid (not refunded, not chargebacked)
                if purchase_info.get("refunded"):
                    return False, "This license has been refunded.", None

                if purchase_info.get("chargebacked"):
                    return False, "This license has been chargebacked.", None

                # Check subscription status if it's a subscription
                if purchase_info.get("subscription_id"):
                    # If there's a subscription, check if it's active
                    if purchase_info.get("subscription_cancelled_at"):
                        return False, "This subscription has been cancelled.", None

                    if purchase_info.get("subscription_failed_at"):
                        return False, "This subscription payment has failed.", None

                return True, "License verified with Gumroad", data

            else:
                error_message = data.get("message", "License verification failed")
                return False, error_message, None

        except requests.exceptions.Timeout:
            return False, "Gumroad API timeout. Please check your internet connection.", None
        except requests.exceptions.RequestException as e:
            return False, f"Network error: Unable to verify with Gumroad. {str(e)}", None
